...
f+"px":"right"===g?c+e+f+"px":!1},calcPosTooltipTop:function(a,b){var c=a.innerHeight(),e=parseInt(a.css("margin-top")),f=b.offset.top||0,g=b.paneltype.position;return"left"===g||"right"===g?-(b.size.height/2)+c/2+e+f+"px":"top"===g?-(b.size.height+f)+e+"px":"bottom"===g?c+e+f+"px":!1},calcToooltipPosition:function(a,b){b.position={top:this.calcPosTooltipTop(a,b),left:this.calcPosTooltipLeft(a,b)};return b.position},calcVerticalOffset:function(a){return Math.floor(a.offset().top-c(window).scrollTop())},
close:function(a){var b=a.parent(),d=a.attr("id"),e,f;c(a).trigger("jspanelbeforeclose",d);this.closeChildpanels(a);b.hasClass("jsPanel-tooltip-wrapper")&&a.unwrap();a.remove();c("body").trigger("jspanelclosed",d);"modal"===a.option.paneltype.type&&c(".jsPanel-backdrop").remove();this.reposMinimized(this.widthForMinimized);"hint"===a.option.paneltype.type&&(f=[jsPanel.hintsTc,jsPanel.hintsTl,jsPanel.hintsTr],f.forEach(function(b){e=b.indexOf(d);-1!==e&&(b.splice(e,1),jsPanel.reposHints(b,a.parentElmtTagname))}));
return b},closeallTooltips:function(){c(".jsPanel-tt").each(function(){c(this).unwrap().remove();c("body").trigger("jspanelclosed",c(this).attr("id"))})},closeChildpanels:function(a){c(".jsPanel",a).each(function(){c(a).trigger("jspanelbeforeclose",c(this).attr("id"));c(this).remove();c("body").trigger("jspanelclosed",c(this).attr("id"))});return a},configControls:function(a){"closeonly"===a.option.controls.buttons?(c(".jsPanel-btn-min, .jsPanel-btn-norm, .jsPanel-btn-max, .jsPanel-btn-small, .jsPanel-btn-smallrev, .jsPanel-btn-hide",
...
parseInt(a.option.maximizedMargin.bottom)+"px"}))},maximize:function(a){c(a).trigger("jspanelbeforemaximize",a.attr("id"));"body"===a.parentElmtTagname||!0===a.option.controls.maxtoScreen?this.maxWithinBody(a):this.maxWithinElement(a);c(a).trigger("jspanelmaximized",a.attr("id"));c(a).trigger("jspanelstatechange",a.attr("id"));return a},minimize:function(a){c(a).trigger("jspanelbeforeminimize",a.attr("id"));a.data({paneltop:parseInt(a.option.position.top),panelleft:parseInt(a.option.position.left),
panelwidth:parseInt(a.option.size.width),panelheight:parseInt(c(".jsPanel-content",a).css("height"))});if("normalized"===a.status||"normalized"===a.option.panelstatus)a.option.size.width=a.outerWidth()+"px",a.option.size.height=a.outerHeight()+"px";a.animate({opacity:0},{duration:400,complete:function(){a.animate({width:jsPanel.widthForMinimized+"px",height:"28px"},{duration:100,complete:function(){jsPanel.movetoMinified(a);jsPanel.resizeTitle(a);a.css("opacity",1)}})}});return a},movetoMinified:function(a){0===
c("#jsPanel-min-container").length&&c("body").append('<div id="jsPanel-min-container"></div>');if("minimized"!==a.status||"minimized"!==a.option.panelstatus)a.css({left:c(".jsPanel","#jsPanel-min-container").length*jsPanel.widthForMinimized+"px",top:0,opacity:1}).appendTo("#jsPanel-min-container").resizable({disabled:!0}).draggable({disabled:!0}),c(a).trigger("jspanelminimized",a.attr("id")),c(a).trigger("jspanelstatechange",a.attr("id"))},normalize:function(a){var b;c(a).trigger("jspanelbeforenormalize",
a.attr("id"));c(window).off("scroll",a.jsPanelfixPos);"minimized"!==a.status&&"minimized"!==a.option.panelstatus||this.restoreFromMinimized(a);b="body"===a.parentElmtTagname?c(window).scrollTop()+a.verticalOffset+"px":a.option.position.top;a.css({width:a.option.size.width,height:a.option.size.height,top:b,left:a.option.position.left});["resizable","draggable"].forEach(function(b){"disabled"!==a.option[b]&&(a[b]("enable"),c(".ui-icon-gripsmall-diagonal-se",a).css({"background-image":"none","text-indent":0}),
c(".ui-resizable-handle",a).css({cursor:""}))});c(a).trigger("jspanelnormalized",a.attr("id"));c(a).trigger("jspanelstatechange",a.attr("id"));"body"===a.parentElmtTagname&&this.fixPosition(a);return a},replaceCSSBottomRight:function(a){var b=a.position();a.css("bottom")&&(a.css({top:parseInt(b.top,10)+"px",bottom:""}),a.option.position.top=parseInt(b.top,10));a.css("right")&&(a.css({left:parseInt(b.left,10)+"px",right:""}),a.option.position.left=parseInt(b.left,10))},reposHints:function(a,b){var d,
e;d="body"===b?c(window).scrollTop():0;a.forEach(function(a){e=c("#"+a);e.animate({top:d+"px"});d+=e.outerHeight(!0)})},reposHintsScroll:function(a){var b=a.offset().top-c(window).scrollTop();c(window).scroll(function(){a.css("top",c(window).scrollTop()+b+"px")})},reposition:function(a,b,d){d&&"string"===typeof d&&(a.option.selector=d,a.appendTo(d),a.parentElmt=c(d).first(),a.parentElmtTagname=a.parentElmt[0].tagName.toLowerCase());"tooltip"!==a.option.paneltype.type&&"hint"!==a.option.paneltype.type&&
(a.option.position=jsPanel.rewriteOPosition(b),a.css({top:"",right:"",bottom:"",left:""}),this.calcPanelposition(a),a.verticalOffset=jsPanel.calcVerticalOffset(a)||0,this.replaceCSSBottomRight(a),"body"===a.parentElmtTagname?this.fixPosition(a):c(window).off("scroll",a.jsPanelfixPos),this.updateOptionPosition(a));return a},reposMinimized:function(){c(".jsPanel","#jsPanel-min-container").each(function(a,b){c(b).animate({left:a*jsPanel.widthForMinimized+"px"})})},resize:function(a,b,c){b&&null!==b?
a.css("width",b):a.css("width",a.content.css("width"));c&&null!==c&&a.css("height",c);this.resizeContent(a);this.resizeTitle(a);this.updateOptionSize(a)},resizeContent:function(a){var b;b="none"===a.footer.css("display")?a.header.outerHeight():a.header.outerHeight()+a.footer.outerHeight();a.content.css({height:a.outerHeight()-b+"px",width:a.outerWidth()+"px"});return a},resizeTitle:function(a){var b=c(a).outerWidth()-c(a.header.controls).outerWidth()-15+"px";a.header.title.css("width",b)},restoreFromMinimized:function(a){"minimized"!==
a.status&&"minimized"!==a.option.panelstatus||a.appendTo(a.option.selector);["resizable","draggable"].forEach(function(b){"disabled"!==a.option[b]&&(a[b]("enable"),c(".ui-icon-gripsmall-diagonal-se",a).css({"background-image":"none","text-indent":0}),c(".ui-resizable-handle",a).css({cursor:""}))});this.reposMinimized(jsPanel.widthForMinimized)},rewriteOPaneltype:function(a){return"modal"===a?{type:"modal",mode:"default"}:"tooltip"===a?{type:"tooltip",position:"top"}:"hint"===a?{type:"hint"}:"modal"===
a.type?{type:"modal",mode:a.mode||"default"}:"tooltip"===a.type?{type:"tooltip",mode:a.mode||!1,position:a.position||!1,shiftwithin:a.shiftwithin||"body",solo:a.solo||!1,cornerBG:a.cornerBG||!1,cornerOX:a.cornerOX||!1,cornerOY:a.cornerOY||!1}:{paneltype:!1}},rewriteOPosition:function(a){if("center"===a)return{top:"center",left:"center"};if("auto"===a)return{top:"auto",left:"auto"};if("top left"===a)return{top:"0",left:"0"};if("top center"===a)return{top:"0",left:"center"};if("top right"===a)return{top:"0",
right:"0"};if("center right"===a)return{top:"center",right:"0"};if("bottom right"===a)return{bottom:"0",right:"0"};if("bottom center"===a)return{bottom:"0",left:"center"};if("bottom left"===a)return{bottom:"0",left:"0"};if("center left"===a)return{top:"center",left:"0"};"center"===a.bottom&&(delete a.bottom,a.top="center");"center"===a.right&&(delete a.right,a.left="center");return a},rewriteOSize:function(a){"string"===typeof a&&"auto"===a&&(a={width:"auto",height:"auto"});return a},setHintDefaults:function(a){a.option.resizable=
...
b.addToolbar=function(a,c){jsPanel.addToolbar(b,a,c);return b};b.control=function(a,c){jsPanel.control(b,a,c);return b};b.resize=function(a,c){jsPanel.resize(b,a,c);return b};b.reposition=function(a,c){jsPanel.reposition(b,a,c);return b};b.reloadContent=function(){b.option.content?b.content.empty().append(b.option.content):b.option.load?(b.content.empty(),jsPanel.load(b)):b.option.ajax?jsPanel.ajax(b):b.option.iframe&&jsPanel.iframe(b)};b.on("click",function(a){a.preventDefault();c(a.target).is("a[href], button, .jsPanel-nofront, .jsPanel-nofront *")||
b.hasClass("jsPanel-modal")||b.css("z-index",jsPanel.setZi(b))});c(".jsPanel-btn-close",b).on("click",function(a){a.preventDefault();jsPanel.close(b,b.parentElmtTagname)});c(".jsPanel-btn-min",b).on("click",function(a){a.preventDefault();jsPanel.minimize(b)});c(".jsPanel-btn-max",b).on("click",function(a){a.preventDefault();jsPanel.maximize(b)});c(".jsPanel-btn-norm",b).on("click",function(a){a.preventDefault();jsPanel.normalize(b)});c(".jsPanel-btn-small",b).on("click",function(a){a.preventDefault();
jsPanel.smallify(b)});c(".jsPanel-btn-smallrev",b).on("click",function(a){a.preventDefault();jsPanel.unsmallify(b)});b.option.paneltype=jsPanel.rewriteOPaneltype(b.option.paneltype);b.option.position=jsPanel.rewriteOPosition(b.option.position);b.option.size=jsPanel.rewriteOSize(b.option.size);if("modal"===b.option.paneltype.type)jsPanel.setModalDefaults(b),1>c(".jsPanel-backdrop").length&&jsPanel.insertModalBackdrop();else if("tooltip"===b.option.paneltype.type){if(jsPanel.setTooltipDefaults(b),b.option.paneltype.solo&&
jsPanel.closeallTooltips(),b.option.position=jsPanel.calcToooltipPosition(b.parentElmt,b.option),b.css({top:b.option.position.top,left:b.option.position.left}),b.parentElmt.parent().hasClass("jsPanel-tooltip-wrapper")||(b.parentElmt.wrap('<div class="jsPanel-tooltip-wrapper">'),b.parentElmt.parent().append(b),"semisticky"===b.option.paneltype.mode?b.hover(function(){c.noop()},function(){jsPanel.close(b)}):"sticky"===b.option.paneltype.mode?c.noop():(b.option.controls.buttons="none",b.parentElmt.off("mouseout"),
b.parentElmt.mouseout(function(){jsPanel.close(b)}))),b.css("overflow","visible"),b.option.paneltype.cornerBG){a=c("<div></div>");var f="jsPanel-corner-"+b.option.paneltype.position,g=parseInt(b.option.paneltype.cornerOX)||0,l=parseInt(b.option.paneltype.cornerOY)||0,d=b.option.paneltype.cornerBG;"bottom"!==b.option.paneltype.position?a.addClass("jsPanel-corner "+f).appendTo(b):a.addClass("jsPanel-corner "+f).prependTo(b);"top"===b.option.paneltype.position?(f=parseInt(b.option.size.width)/2-12+g+
"px",a.css({borderTopColor:d,left:f})):"right"===b.option.paneltype.position?(f=parseInt(b.option.size.height)/2-12+l+"px",a.css({borderRightColor:d,left:"-22px",top:f})):"bottom"===b.option.paneltype.position?(f=parseInt(b.option.size.width)/2-12+g+"px",a.css({borderBottomColor:d,left:f,top:"-22px"})):"left"===b.option.paneltype.position&&(f=parseInt(b.option.size.height)/2-12+l+"px",a.css({borderLeftColor:d,left:b.option.size.width,top:f}))}}else"hint"===b.option.paneltype.type&&(jsPanel.setHintDefaults(b),
c(".jsPanel-hint-close-dark, .jsPanel-hint-close-white",b).on("click",b,function(a){a.data.close(b)}),"0"===b.option.position.top&&"center"===b.option.position.left?(0<jsPanel.hintsTc.length&&(b.option.position=jsPanel.hintTop(jsPanel.hintsTc)),jsPanel.hintsTc.push(b.attr("id"))):"0"===b.option.position.top&&"0"===b.option.position.left?(0<jsPanel.hintsTl.length&&(b.option.position=jsPanel.hintTop(jsPanel.hintsTl)),jsPanel.hintsTl.push(b.attr("id"))):"0"===b.option.position.top&&"0"===b.option.position.right&&
(0<jsPanel.hintsTr.length&&(b.option.position=jsPanel.hintTop(jsPanel.hintsTr)),jsPanel.hintsTr.push(b.attr("id"))));b.data("selector",b.option.selector);"tooltip"!==b.option.paneltype.type&&b.appendTo(b.parentElmt);"modal"===b.option.paneltype.type?(b.css("zIndex",10001),"extended"===b.option.paneltype.mode&&c(".jsPanel-backdrop").css("z-index","9998")):b.hasClass("jsPanel-modal")||b.css("z-index",jsPanel.setZi(b));b.option.bootstrap?(b.option.theme=jsPanel.isBootstrapTheme(b.option.bootstrap),b.option.controls.iconfont=
"bootstrap",b.alterClass("jsPanel-theme-*","panel panel-"+b.option.theme),b.header.alterClass("jsPanel-theme-*","panel-heading"),b.header.title.addClass("panel-title"),b.content.alterClass("jsPanel-theme-*","panel-body"),b.footer.addClass("panel-footer"),b.header.title.css("color",function(){return b.header.css("color")}),b.content.css("border-top-color",function(){return b.header.css("border-top-color")})):[b,b.header,b.content,b.footer].forEach(function(a){c(a).alterClass("jsPanel-theme-*","jsPanel-theme-"+
b.option.theme)});b.option.removeHeader?b.header.remove():jsPanel.configControls(b);b.option.controls.iconfont?jsPanel.configIconfont(b):c(".jsPanel-btn-close, .jsPanel-btn-max, .jsPanel-btn-norm, .jsPanel-btn-min, .jsPanel-btn-small, .jsPanel-btn-smallrev",b.header.controls).empty();b.option.toolbarHeader&&!1===b.option.removeHeader&&("string"===typeof b.option.toolbarHeader?b.header.toolbar.append(b.option.toolbarHeader):c.isFunction(b.option.toolbarHeader)?b.header.toolbar.append(b.option.toolbarHeader(b.header)):
