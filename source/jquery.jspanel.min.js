/* jQuery Plugin jsPanel version 2.5.4 - Copyright (c) 2014-15 Stefan Str‰ﬂer, <http://jspanel.de/> <http://opensource.org/licenses/MIT> */
...
a.header.title.css("width","100%"));["close","maximize","minimize","normalize","smallify"].forEach(function(c){a.option.controls[c]&&a.control("disable",c)})},configIconfont:function(a){var c="close max norm min small smallrev".split(" "),b="remove fullscreen resize-full minus chevron-up chevron-down".split(" "),d="times arrows-alt expand minus chevron-up chevron-down".split(" "),e=a.option.controls.iconfont,f=a.header.controls;$("*",f).css("background-image","none");"bootstrap"===e?c.forEach(function(a,
c){$(".jsPanel-btn-"+a,f).empty().append('<span class="glyphicon glyphicon-'+b[c]+'"></span>')}):"font-awesome"===e&&c.forEach(function(a,b){$(".jsPanel-btn-"+a,f).empty().append('<span class="fa fa-'+d[b]+'"></span>')})},configToolbar:function(a,c,b){var d;a.forEach(function(a){if("object"===typeof a&&(d=$(a.item),"string"===typeof a.btntext&&d.append(a.btntext),"string"===typeof a.btnclass&&d.addClass(a.btnclass),c.append(d),$.isFunction(a.callback)))d.on(a.event,b,a.callback)})},control:function(a,
c,b){var d=a.header.controls,e;3===arguments.length&&("disable"===arguments[1]?("close"===b?e=$(".jsPanel-btn-close",d):"maximize"===b?e=$(".jsPanel-btn-max",d):"minimize"===b?e=$(".jsPanel-btn-min",d):"normalize"===b?e=$(".jsPanel-btn-norm",d):"smallify"===b&&(e=$(".jsPanel-btn-small",d)),e.off().css({opacity:.5,cursor:"default"})):"enable"===arguments[1]&&("close"===b?e=$(".jsPanel-btn-close",d):"maximize"===b?e=$(".jsPanel-btn-max",d):"minimize"===b?e=$(".jsPanel-btn-min",d):"normalize"===b?e=
$(".jsPanel-btn-norm",d):"smallify"===b&&(e=$(".jsPanel-btn-small",d)),e.on("click",function(c){c.preventDefault();a[b]()}).css({opacity:1,cursor:"pointer"})));return a},dblclickhelper:function(a,c){if("string"===typeof a)if("maximize"===a||"normalize"===a)"normalized"===c.status||"normalized"===c.option.panelstatus?c.maximize():c.normalize();else if("minimize"===a||"smallify"===a||"close"===a)c[a]()},exportPanels:function(){var a,c,b,d,e,f,h,l=[],k;$(".jsPanel").not(".jsPanel-tt, .jsPanel-hint, .jsPanel-modal").each(function(m,
g){k={panelstatus:$(g).data("panelstatus"),id:$(g).prop("id"),title:$(".jsPanel-title",g).html(),custom:$(g).data("custom"),offset:{top:0,left:0},content:$(g).data("content")};"normalized"!==$(g).data("panelstatus")&&$(".jsPanel-btn-norm",g).trigger("click");h=$(g).data("selector");a=$(g).offset();c=$(g).position();"body"===h.toLowerCase()?(b=Math.floor(a.top-$(window).scrollTop())+"px",d=Math.floor(a.left-$(window).scrollLeft())+"px"):(b=Math.floor(c.top)+"px",d=Math.floor(c.left)+"px");e=$(g).css("width");
f=$(".jsPanel-content",g).css("height");k.size={width:e,height:f};k.position={top:b,left:d};$(g).data("loadURL")&&(k.load={},k.load.url=$(g).data("loadURL"));$(g).data("ajaxURL")&&(k.ajax={},k.ajax.url=$(g).data("ajaxURL"));if($(g).data("iframeDOC")||$(g).data("iframeSRC"))k.iframe={},$(g).data("iframeDOC")&&(k.iframe.srcdoc=$(g).data("iframeDOC")),$(g).data("iframeSRC")&&(k.iframe.src=$(g).data("iframeSRC"));l.push(k);switch(k.panelstatus){case "minimized":$(".jsPanel-btn-min",g).trigger("click");
break;case "maximized":$(".jsPanel-btn-max",g).trigger("click");break;case "smallified":$(".jsPanel-btn-small",g).trigger("click");break;case "smallifiedMax":$(".jsPanel-btn-small",g).trigger("click")}});window.localStorage.setItem("jspanels",JSON.stringify(l));return l},importPanels:function(a){var c,b,d;c=JSON.parse(localStorage.jspanels)||{};d=a["default"]||{};c.forEach(function(c){b="string"===typeof c.custom.config?$.extend(!0,{},d,a[c.custom.config],c):$.extend(!0,{},d,c);$.jsPanel(b)})},fixPosition:function(a){var c=
a.offset().top-$(window).scrollTop();a.jsPanelfixPos=function(){a.css("top",$(window).scrollTop()+c)};$(window).on("scroll",a.jsPanelfixPos)},getMargins:function(a){var c,b,d,e,f=a.option.paneltype.shiftwithin;e=$(window).outerWidth();b=$(window).outerHeight();var h=a.outerWidth(),l=a.outerHeight();f&&"body"!==f?(b=$(f).offset(),c=a.offset(),d=$(f).outerWidth()-parseInt(a.css("width"))-(c.left-b.left),e=c.left-b.left,a=$(f).outerHeight()-(c.top-b.top)-parseInt(a.css("height")),c=c.top-b.top):(c=a.offset(),
d=e-c.left-h+$(window).scrollLeft(),e=e-h-d,a=b-c.top-l+$(window).scrollTop(),c=b-l-a);return{marginTop:parseInt(c),marginRight:parseInt(d),marginBottom:parseInt(a),marginLeft:parseInt(e)}},getMaxOfArray:function(a){return Math.max.apply(null,a)},getMaxpanelshift:function(a){var c=a.outerWidth(),b=a.outerHeight(),d=parseInt(c/2)+parseInt(a.parent().outerWidth()/2)-20;a=parseInt(b/2)+parseInt(a.parent().outerHeight()/2)-20;c=parseInt(c/2)-16;b=parseInt(b/2)-16;return{maxshiftH:d,maxshiftV:a,maxCornerH:c,
